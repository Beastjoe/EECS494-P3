...
public class ArrowKeyMovementDummy : MonoBehaviour {
    ...
    private void OnCollisionEnter(Collision collision) {
        ...
        {
            GameObject tutorial = GameObject.Find("Canvas/Tutorial6.5_Defense");
            if (tutorial != null && tutorial.activeSelf)
            {
                if (collision.gameObject.GetComponent<playerStatus>().teamIdx == 0)
                {
                    tutorial.GetComponent<TutorialDummyBurrowController>().redHit = true;
                }
                if (collision.gameObject.GetComponent<playerStatus>().teamIdx == 1)
                {
                    tutorial.GetComponent<TutorialDummyBurrowController>().blueHit = true;
                }
            }
        }
    }
